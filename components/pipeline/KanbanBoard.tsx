...
function KanbanColumn({ id, label, color, cards, onCardOpen }: KanbanColumnProps) {
    ...
    <div className="flex flex-col w-72 shrink-0 h-full gap-2">
      ...
      >
        <SortableContext items={cards.map((c) => c.cp_id)} strategy={verticalListSortingStrategy}>
          {cards.map((card) => (
            <KanbanCard key={card.cp_id} card={card} onOpen={onCardOpen} />
          ))}
        </SortableContext>
        {cards.length === 0 && (
...
export function KanbanBoard({ data, onCardOpen, isLoading }: KanbanBoardProps) {
  ...
  // Build columns with optimistic overrides applied
